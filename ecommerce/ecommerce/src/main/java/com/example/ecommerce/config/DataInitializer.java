...
public class DataInitializer implements CommandLineRunner {
    ...
    private void seedUsers() {
        ...
        if (adminUserOptional.isEmpty()) {
            ...
            adminUser = userService.createUser(adminUser, "ROLE_ADMIN");
            System.out.println("Admin user 'admin2' created with password 'admin'.");
        } else {
            adminUser = adminUserOptional.get();
        }

        // This line ensures a cart is created if it doesn't exist, even if the user already does.
        createCartForUser(adminUser);
    }
    ...
    private void createCartForUser(User user) {
        if (cartRepository.findByUser(user).isEmpty()) {
            ...
            System.out.println("Cart created for user: " + user.getUsername());
        }
    }
